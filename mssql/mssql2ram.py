...
class MSSQLDownloader:
    def __init__(self,queries):
        ...
        self.conn = pyodbc.connect('Driver={SQL Server};'
                      'Server=DESKTOP-SCQKLN2\SQLEXPRESS;'
                      'Database=Northwind;'
    ...
    def load_table(self, schema:Schema):
        ...
        for row in rows:
            ...
            table.indexes = self.load_indixes(schema.name, table.name)
    ...
    def load_indixes(self, schema_name:str, table_name:str):
        self.cursor.execute(self.queries.get_indixes, "{0}.{1}".format(schema_name,table_name))
        rows = self._get_result()
        list = []
        for row in rows:
            index = Index()
    ...
    def load_fields(self, table_name:str):
        ...
        for row in rows:
            ...
            domain.type =type_[domain.type.upper()]    #get_type_in_postgres(domain)
            field.domain = domain
            list.append(field)
        return list
